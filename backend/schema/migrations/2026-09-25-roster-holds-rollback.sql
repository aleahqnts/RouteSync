-- Backs out 2026-09-25-roster-holds.sql.
--
-- Deploy a dashboard without holds first. One that still sends them calls save_roster_month
-- with p_held, and the four-argument function put back here does not take it, so every save
-- would be refused.
--
-- Destructive for holds only: every driver held back from any month is released, and the
-- next auto-fill may place them. Seats, floaters and rest days are untouched.

begin;

drop function if exists public.save_roster_month(date, integer, jsonb, integer, jsonb);
...
drop table if exists public.roster_holds;
